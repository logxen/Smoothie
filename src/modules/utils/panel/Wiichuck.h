...
public:
    ...
    void init_device(){
        ...
        this->i2c.write(0xFA);
        this->i2c.stop();
        int res = this->i2c.read(i2c_address, data_buf, 6)
        if(res == 0) {
            if(data_buf[2] == 0xA4) {
    ...
    void poll_device() {
        ...
        if(device_type >= 0) {
            switch(device_type) {
            case DEVICE_NUNCHUCK:       poll_nunchuck();
                break;
            case DEVICE_CLASSIC:        poll_classic();
                break;
            case DEVICE_INACTIVE_WMP:   init_wmp();
                break;
            case DEVICE_WMP:            poll_wmp();
                break;
            default:
                break;
            }
    ...
    int read_device(char mem_address) {
        this->i2c.write(i2c_address, mem_address, 1);
        return this->i2c.read(i2c_address, data_buf, 6);
    }

    void poll_nunchuck() {
        if(read_device(0x00) == 0) {
            SX = data_buf[0];
            SY = data_buf[1];
